...
function classify(name: string): Runtime | null {
  const n = name.toLowerCase()
  if (n === 'node' || n === 'node.exe' || n.includes('node')) return 'node'
  if (n === 'java' || n === 'java.exe' || n.includes('java')) return 'java'
  return null
}
...
export class PortManager {
  ...
  async killProcess(pid: number): Promise<KillResult> {
  ...
  // ── Windows ─────────────────────────────────────────────────────
  private async _listWindows(): Promise<PortInfo[]> {
  ...
  private async _tasklistNames(pids: number[]): Promise<Map<number, string>> {
    ...
    for (const pid of [...map.keys()]) if (!wanted.has(pid)) map.delete(pid)
    return map
  }

  // ── macOS / Linux ───────────────────────────────────────────────
  private async _listUnix(): Promise<PortInfo[]> {
    ...
    for (const line of stdout.split(/\r?\n/)) {
      ...
      if (parts[7]?.toUpperCase() !== 'TCP') continue
   ...
   */
  private _splitAddress(token: string): { address: string; port: number | null } {
    const idx = token.lastIndexOf(':')
    if (idx === -1) return { address: token, port: null }
